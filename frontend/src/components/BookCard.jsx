...
const BookCard = ({ book, rating = 0 }) => {
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      try {
        ...
        const cartIds = res.data.data.map((item) => item.book._id);
        setIsInCart(cartIds.includes(book.id));
      } catch (error) {
  ...
  const handleWishlistClick = async () => {
    try {
      const res = await api.post("/api/v1/users/toggle-wishlist", {
        bookId: book.id,
      });
  ...
  const handleAddToCart = async () => {
    try {
      await api.post("/api/v1/cart/add", { bookId: book.id, quantity: 1 });
      setIsInCart(true);
  ...
  const handleRemoveFromCart = async () => {
    try {
      await api.post("/api/v1/cart/remove", { bookId: book.id });
      setIsInCart(false);
      setToastMsg("Book removed from cart");
    } catch (err) {
      console.error("Error removing from cart:", err);
      setToastMsg("Failed to remove from cart");
    }
    setTimeout(() => setToastMsg(null), 2000);
  };
  ...
  const handleBuyNow = async () => {
    ...
    sessionStorage.setItem("buyBook", JSON.stringify(book));
  ...
  return (
    <>
      <div className="relative p-4 rounded-lg border border-gray-200 shadow-md bg-white w-60">
        ...
        <div className="font-parastoo text-base text-gray-600">
          {book.author}
        </div>
            ...
            <div className="w-2/3 space-y-2">
              <div className="flex justify-between items-start">
                ...
                <button onClick={handleWishlistClick}>
                  {isWishlisted ? (
                    <FaHeart className="text-red-700 text-xl mr-4" />
                  ) : (
              ...
              <div className="flex gap-2 mt-4">
                <button
                  disabled={book.count === 0 || isInCart}
      ...
      {toastMsg && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white border border-green-500 px-4 py-2 rounded shadow-md flex items-center gap-4 z-[999]">
          <span className="font-parastoo text-lg text-gray-800">
            {toastMsg}
          </span>
        </div>
      )}
