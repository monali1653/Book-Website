...
const addOrUpdateRating = asyncHandler(async (req, res) => {
  ...
  if (existing) {
    ...
    await existing.save();
    return res.status(200).json(new ApiResponse(200, existing, "Rating updated"));
  }
  ...
  return res.status(201).json(new ApiResponse(201, newRating, "New Rating added"));
});
...
const getBookRatings = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, result, "All book ratings fetched successfully"));
});

export { addOrUpdateRating, getUserRatings, getBookRatings };